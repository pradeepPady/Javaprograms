...
public class ClinicManagementApp {
	ArrayList<Appointment>appointList=new ArrayList<Appointment>();
	ArrayList<Doctor>DoctorsList;
	ArrayList<Patient>pationtList;
	...
	public void searchDetails() throws FileNotFoundException
	{
				...
				switch (choice) 
				{
				case 1:arrayList=DoctorSearchMenu();
				break;
...
public void saveAppoint() throws FileNotFoundException {
		...
		ArrayList<Appointment>pationtList1=readAppointFile();
		if(pationtList1.size()>0)
		appointList.addAll(pationtList1);
		try {
			mapper.writeValue(new FileOutputStream(patientFile), appointList);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void appointment() throws FileNotFoundException 
	{
		...
		boolean flag=true;
		back:for(int i=0;i<array.size();i++)
		{
			if(array.get(i).getName().equals(name)&&array.get(i).getId()==id)
			{
				...
				boolean flag1=true;
					...
					{
						if(array1.get(i).getName().equals(docterName)&&array1.get(i).getId()==docterId)
						{
							if(array1.get(i).getCount()<5)
							{
								...
								appointment.setDocterAvalability(array1.get(i).getAvalability());
								  
								appointment.setDate(java.time.LocalDate.now());
								array1.get(i).setCount((array1.get(i).getCount()+1));
								array1.get(i).setNumberOfPatients((array1.get(i).getNumberOfPatients()+1));
								flag1=false;
								flag=false;
